...
(function() {
  ...
  function FrogRunnerSprite(){
      ...
      let animations = {
        ...
        jump: new Gamelab.Animation(filePath),
      ...
      $actionSprite.cubic_jump = function (height = 200, totalSteps) {
        ...
        for (var x = 0; x <= 1.0; x += step) {
          var pct = CubicOut(x);
          vpoints.push(new Gamelab.Vector(0, this.position.y - (pct * height)));
        }
      ...
      $actionSprite.runLeft = function(speed){

          if(!this.__inAir)
          this.selected_animation = animations.run_gun_straight;
          this.flipX = true;
          this.speed.x = Math.abs(speed) * -1;

          this.collide_stop_left_right(gameLevel.terrains);

      };

      $actionSprite.runRight = function(speed){

          if(!this.__inAir)
          this.selected_animation = animations.run_gun_straight;
          this.flipX = false;
          this.speed.x = Math.abs(speed);

          this.collide_stop_left_right(gameLevel.terrains);

      };

      $actionSprite.jumpPoints = $actionSprite.jumpPoints || [];

      $actionSprite.jump = function(height){

          if(this.jumpPoints.length == 0 && !this.__inAir)
          {
            this.jumpPoints = this.cubic_jump(height, 20);
            this.__inAir = true;
          }
      ...
      for (var x in animations) {
        ...
        animations[x].onLoad(function (anime) {
          ...
          if (this.key_name == 'run_gun_straight') {
            this.FrameSize(200, 160);
            ...
            this.Position(0, 0);
                    ...
                    if (this.key_name == 'jump') {
                      this.FrameSize(200, 160);
                      this.FrameOffset(90, -20).FrameBounds(new Gamelab.Vector(1, 4), new Gamelab.Vector(1, 4), new Gamelab.Vector(1, 4))
                      this.Size(200, 160);
                      this.Position(0, 0);
                    }
